% -------------------------------------------------------------------------
%  CONTROL ANALYSIS – Surrogate Inter-Brain *Concurrent* Coordination (GCMI)
%
...
%  from different dyads. For each surrogate pairing, computes concurrent GCMI 
...
%      7. Extract peak concurrent MI
%      8. Export surrogate datasets for downstream analysis
%
%  NOTES:
%      - This version computes concurrent (near-instantaneous) GCMI only.
...
for metasur = nmetasurs
    ...
    T = table('Size', [0 7], ...
                ...
                'VariableNames', {'Trial','Channel_A','Channel_B', 'SyncMI', 'FrequencyBand', 'Subject_A', 'Subject_B'});
    ...
    for dyadi = 1:size(pairs,1)
        ...
        sub_A = hyper_epoching(nameA);
        sub_B = hyper_epoching(nameB);
        ...
        % Define which are conversations
        ...
        %% Add non-conversation trials for each frequency band (if wanted, but not used in manuscript)
        ...
        %% Add signal length to the right for later normalization (not used anymore)
        ...
        for b = 1:numBands
            ...
            %  parfor to loop over trials ( trial is independent)
            parfor bi = 1:Nblock
            ...
            for bi = 1:Nblock
                % compute the effective signal length using the l=0 trimming
                samples2trim = L_max;
                idx_start = ceil(samples2trim/2) + 1;
        ...
        for b = 1:numBands
            ...
            TTemp = table(trial_flat, chanA_flat, chanB_flat, MI_flat, ...
                      'VariableNames', {'Trial', 'Channel_A', 'Channel_B', 'SyncMI'});
    ...
    filename = fullfile('output', '10b_shuffled_dyads', ['gcmi_sync_shuffled_surrogates_', num2str(metasur) ,'.csv']);
